...
func main() {
	...
	for i:=0;i<1000;i++{
		_, err := pg.NewClient(config.PgConfig{
			...
			Sslmode:        "disable",
			MaxOpenConns:   10,
			MigrationsDir:  "file://./db/migration/games",
			MigrateVersion: 1,
			JsonDir:        "./json/",
		})
		if err != nil {
	...
	for i:=0;i<1000;i++{
		_, err := pg.NewClient(config.PgConfig{
			...
			Sslmode:        "disable",
			MaxOpenConns:   10,
			MigrationsDir:  "file://./db/migration/users",
			MigrateVersion: 1,
			JsonDir:        "",
		})
		if err != nil {
	...
	for;; {
		res, er1 := pg.GetTotalSellByGenre(context.Background(), 0, int(time.Now().Unix()))
		if er1 == nil && len(res) > 0 {
			break
		}
		log.Info().Msgf("Failed to connect to get games")
		time.Sleep(10 * time.Second)
	}
	var c config.Config
